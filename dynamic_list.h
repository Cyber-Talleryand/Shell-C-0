...
#include "types.h"              //Añadimos el contenido del fichero "types.h".
...
struct tNode{                                       //Declaración de una estructura con una variable "struct(tItemL)" denominada "data" y otra
    tItemL data;                                    //variable "tPosL" que es un puntero a un nodo denominado "next".
    tPosL next;};
...
bool insertItem(tItemL d, tList* L);                //Se posiciona el item en el lugar correspondiente.
bool copyList(tList L, tList* M);                   //Pasa una copia de la lista copiada y la dirección de la lista a copiar.
void updateItem(tItemL d , tPosL p, tList* L);      //Modifica el contenido del elemento situado en la posición indicada.
...
tPosL findItem(tItemL d, tList L);               //Devuelve la posición del primer elemento de la lista cuyo nick de usuario
...
tItemL getItem(tPosL p, tList L);                   //Devuelve el contenido del elemento de la lista que ocupa la posición indicada.
